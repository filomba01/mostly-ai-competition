...
class BatchCollator:
    ...
    def __init__(self, is_sequential: bool, max_sequence_window: int | None, device: torch.device, flip_start_sequence: float = 0.3, flip_end_sequence: float = 0.4):
        self.is_sequential = is_sequential
        self.max_sequence_window = max_sequence_window
        self.device = device
        self.flip_start_sequence = flip_start_sequence
        self.flip_end_sequence = flip_end_sequence

    def __call__(self, batch: list[dict]) -> dict[str, torch.Tensor]:
        batch = pd.DataFrame(batch)
        if self.is_sequential and self.max_sequence_window:
            batch = self._slice_sequences(batch, self.max_sequence_window, flip_start_sequence=self.flip_start_sequence, flip_end_sequence=self.flip_end_sequence)
    ...
    @staticmethod
    def _slice_sequences(batch: pd.DataFrame, max_sequence_window: int, flip_start_sequence: float = 0.3, flip_end_sequence: float = 0.4) -> pd.DataFrame:
        ...
        if flip < flip_start_sequence:  # 30%
        ...
        elif flip_start_sequence <= flip < flip_end_sequence:  # 10%
    ...
    *,
    ...
    with ProgressCallbackWrapper(
        ...
        batch_collator = BatchCollator(
            is_sequential=is_sequential, max_sequence_window=max_sequence_window, device=device, flip_start_sequence=flip_start_sequence, flip_end_sequence=flip_end_sequence
        )
        ...
        early_stopper = EarlyStopper(val_loss_patience=val_loss_patience)#4)
        optimizer = torch.optim.AdamW(params=argn.parameters(), lr=initial_lr, weight_decay=weight_decay)
