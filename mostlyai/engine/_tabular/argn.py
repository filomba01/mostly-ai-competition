...
def _embedding_heuristic(id: str, model_size: ModelSizeOrUnits, dim_input: int, embedding_scale_factor: float = 2.8, embedding_exponent: float = 0.23, embedding_lower_bound: int = 12, exp_embedding_scale_factor: float = 0.25) -> int:
    if isinstance(model_size, dict):
        return model_size[id]

    model_size_output_dim = dict(
        S=max(embedding_lower_bound, int(2 * np.ceil(dim_input**0.15))),
        M=max(embedding_lower_bound, int(3 * np.ceil(dim_input**exp_embedding_scale_factor))),
        L=max(embedding_lower_bound, int(4 * np.ceil(dim_input**0.33))),
        C=max(embedding_lower_bound, int(embedding_scale_factor * np.ceil(dim_input**embedding_exponent))),
...
def _column_embedding_heuristic(
    ...
    column_embedding_lower_bound: int = 8,
) -> int:
    if isinstance(model_size, dict):
    ...
    model_size_output_dim = dict(
        ...
        M=int(10 + n_sub_cols),
        L=int(16 + n_sub_cols),
        C=int(column_embedding_lower_bound + n_sub_cols),
    )
...
def _regressor_heuristic(id: str, model_size: ModelSizeOrUnits, dim_input: int, cardinality: int, layer_size: list[int] = [16]) -> list[int]:
    if isinstance(model_size, dict):
        return model_size[id]
    model_size_layers = dict(S=[4], M=[16], L=[16, 16], C=layer_size)
    dims = [dim_input]
...
def _flat_context_heuristic(id: str, model_size: ModelSizeOrUnits, dim_input: int, layer_size: list[int] = [64]) -> list[int]:
    if isinstance(model_size, dict):
        return model_size[id]
    assert isinstance(layer_size, list), "Layer size in flat context must be a list of integers"
    assert len(layer_size) == 1, "Layer size in flat context must be a list of integers with single element"
    assert layer_size[0] > 0, "Layer size in flat context must be a positive integer"
    
    model_size_layers = dict(S=[8], M=[64], L=[128], C=layer_size)
    layers = model_size_layers[model_size]
...
def _sequential_context_heuristic(
    id: str, model_size: ModelSizeOrUnits, dim_input: int, seq_len_median: int, layer_size: list[int] = [32]
) -> list[int]:
    if isinstance(model_size, dict):
        return model_size[id]
    assert isinstance(layer_size, list), "Layer size in sequential context must be a list of integers"
    model_size_layers = dict(S=[8], M=[32], L=[64, 64], C=layer_size)
    layers = model_size_layers[model_size]
...
def _history_heuristic(id: str, model_size: ModelSizeOrUnits, dim_input: int, seq_len_median: int, layer_size: list[int] = [64]) -> list[int]:
    if isinstance(model_size, dict):
        return model_size[id]
    assert isinstance(layer_size, list), "Layer size in history must be a list of integers"
    model_size_layers = dict(S=[16], M=[64], L=[128, 128], C=layer_size)
    layers = model_size_layers[model_size]
    coefficient = round(np.log(max(dim_input * seq_len_median, np.e)))
    dims = [unit * coefficient for unit in layers]
    return dims
...
class Embedders(nn.Module):
    def __init__(
        ...
        for sub_col, dim_input in self.cardinalities.items():
            dim_output = _embedding_heuristic(id=self.id(sub_col), model_size=model_size, dim_input=dim_input, embedding_lower_bound=embedding_lower_bound, embedding_scale_factor=embedding_scale_factor, embedding_exponent=embedding_exponent, exp_embedding_scale_factor=exp_embedding_scale_factor)
...
class SequentialContextEmbedders(Embedders):
    def __init__(
        ...
        super().__init__(model_size, cardinalities, device, embedding_lower_bound, embedding_scale_factor, embedding_exponent, exp_embedding_scale_factor)
...
class FlatContextCompressor(nn.Module):
    def __init__(
        ...
        if self.embedders:
            ...
            dims = _flat_context_heuristic(id=self.id(), model_size=model_size, dim_input=dim_input, layer_size=layer_size)
...
class ContextCompressor(nn.Module):
    def __init__(
        ...
        self.flat_context_compressor = FlatContextCompressor(
            ...
            layer_size=layer_size_flat_context,
            embedding_lower_bound=embedding_lower_bound,
            embedding_scale_factor=embedding_scale_factor,
            embedding_exponent=embedding_exponent,
        )
        ...
        self.sequential_context_compressor = SequentialContextCompressor(
            ...
            with_dp=with_dp,
            dropout_rate=dropout_rate,
            layer_size=layer_size_sequential_context,
        )
...
class FlatModel(nn.Module):
    def __init__(
        ...
        self.context_compressor = ContextCompressor(
            ...
            ctxseq_len_median=self.ctxseq_len_median,
            device=device,
            dropout_rate=dropout_rate_context,
        ...
        self.regressors = Regressors(
            ...
            device=self.device,
            dropout_rate=dropout_rate_regressor,
            layer_size=layer_size_regressor,
        )
...
class SequentialModel(nn.Module):
    def __init__(
        ...
        self.history_compressor = HistoryCompressor(
            ...
            device=self.device,
            with_dp=with_dp,
            dropout_rate=dropout_rate_history,
            layer_size=layer_size_history,
        )
